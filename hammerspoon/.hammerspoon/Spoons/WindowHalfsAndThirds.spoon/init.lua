...
obj._window_state_name_to_rect = {
	...
	top_left_third = { 0.00, 0.00, 0.33, 0.50 },
	top_right = { 0.50, 0.00, 0.50, 0.50 },
	top_right_third = { 0.67, 0.00, 0.33, 0.50 },
	bottom_left = { 0.00, 0.50, 0.50, 0.50 },
	bottom_left_third = { 0.00, 0.50, 0.33, 0.50 },
	bottom_right = { 0.50, 0.50, 0.50, 0.50 },
	bottom_right_third = { 0.67, 0.50, 0.33, 0.50 },
...
obj._window_moves = {
	...
	third_left = {
		...
		middle_third_h = "left_third",
		right_third = "middle_third_h",
		right_half = "middle_third_h",
	},
	third_right = {
		"right_third",
		left_third = "middle_third_h",
		middle_third_h = "right_third",
		right_third = "left_third",
	...
	third_up = {
		...
		middle_third_v = "top_third",
		bottom_third = "middle_third_v",
		bottom_half = "middle_third_v",
	},
	third_down = {
		"bottom_third",
		top_third = "middle_third_v",
		middle_third_v = "bottom_third",
		bottom_third = "top_third",
...
function obj.resizeCurrentWindow(how, use_frame_correctness_preference)
	...
	local move_to = obj._lastMoveCache[win:id()] and obj._window_moves[how][obj._lastMoveCache[win:id()]]
		or obj._window_moves[how][current_window_state_name(win)]
		or obj._window_moves[how][1]
	if not move_to then
		obj.logger.e(
			"I don't know how to move "
				.. how
				.. " from "
				.. (obj._lastMoveCache[win:id()] or current_window_state_name(win))
		)
	end
	if current_window_state_name(win) == move_to then
		return
	end
	...
	if use_frame_correctness_preference then
		_setFrameCorrectness()
	end
	cacheWindow(win, move_to)
	win:move(move_to_rect)
	if use_frame_correctness_preference then
		_restoreFrameCorrectness()
	end
end
...
obj.topLeftThird = hs.fnutils.partial(obj.resizeCurrentWindow, "top_left_third")
obj.topRight = hs.fnutils.partial(obj.resizeCurrentWindow, "top_right")
obj.topRightThird = hs.fnutils.partial(obj.resizeCurrentWindow, "top_right_third")
obj.bottomLeft = hs.fnutils.partial(obj.resizeCurrentWindow, "bottom_left")
obj.bottomLeftThird = hs.fnutils.partial(obj.resizeCurrentWindow, "bottom_left_third")
obj.bottomRight = hs.fnutils.partial(obj.resizeCurrentWindow, "bottom_right")
obj.bottomRightThird = hs.fnutils.partial(obj.resizeCurrentWindow, "bottom_right_third")
...
function obj:bindHotkeys(mapping)
	local action_to_method_map = {
		...
		top_left_third = self.topLeftThird,
		top_right = self.topRight,
		top_right_third = self.topRightThird,
		bottom_left = self.bottomLeft,
		bottom_left_third = self.bottomLeftThird,
		bottom_right = self.bottomRight,
		bottom_right_third = self.bottomRightThird,
		undo = self.undo,
		center = self.center,
		larger = self.larger,
...
function obj:init()
	self._frameCache = {}
	obj._lastMoveCache = {}
	self._frameCacheClearTimer = hs.timer.delayed.new(obj.clear_cache_after_seconds, function()
		obj._frameCache = {}
		obj._lastMoveCache = {}
	end)
end
